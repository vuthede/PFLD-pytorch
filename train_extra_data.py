...
from models.resnet import resnet101PFLD, resnet101BottleNeckPFLD
import mxnet as mx
...
wandb.config.pfld_backbone = "ResNet101BotteNeck" # It is customized for PFLD  
...
wandb.config.data_file_train_rec = "/home/vuthede/AI/PFLD-pytorch/data/pfld_train_data_extra.rec"
wandb.config.data_file_valid_rec = "/home/vuthede/AI/PFLD-pytorch/data/pfld_test_data.rec"
...
def train(train_iter, plfd_backbone, auxiliarynet, criterion, optimizer,
    ...
    for batch in train_iter:    
        ...
        landmark_gt = labels[:, 0:98*2]
        attribute_gt = labels[:, 2*98+1:2*98+6]
        euler_angle_gt = labels[:, -4:-1] 
        ...
        # Beacause data is combine between 98 lmks and 68 lmks
        # Then sample with 68 lmks will have (98-68)==30 lmks with values == -1 or minus value
        # Then I will mask it by the gt data, so that the loss in those lmks==0
        # print(landmark_gt.shape, landmarks.shape)
        # print(landmark_gt)
        # print(landmarks)

        landmark_gt = torch.where(landmark_gt<0, landmarks, landmark_gt)
...
def validate(valid_iter, plfd_backbone, auxiliarynet, criterion):
    plfd_backbone.eval()
    auxiliarynet.eval() 
    losses = []
    nme_list = []
...
def main(args):
    ...
    train_data_file = wandb.config.data_file_train_rec
    image_size=112
    ...
    valid_data_file = wandb.config.data_file_valid_rec
    image_size=112
    ...
    for epoch in range(args.start_epoch, args.end_epoch + 1):
        ...
        weighted_train_loss, train_loss = train(train_iter, plfd_backbone, auxiliarynet,
        ...
        val_loss = validate(valid_iter, plfd_backbone, auxiliarynet,
                            criterion)
